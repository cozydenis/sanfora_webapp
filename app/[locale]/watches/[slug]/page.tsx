...
import { getProductBySlug, formatPrice, getWatches } from '@/lib/products';
import { isWatchProduct } from '@/lib/types';
...
export async function generateMetadata({ params: { locale, slug } }: Props): Promise<Metadata> {
  ...
  if (!product || !isWatchProduct(product)) {
    return {};
  }
  ...
  return genMeta({
    ...
    path: `/${locale}/watches/${slug}`,
    locale: localeTyped,
    images: product.images,
  });
}

export default async function WatchDetailPage({ params: { locale, slug } }: Props) {
            ...
            <div>
              ...
              <div className="mb-10">
                ...
                <div className="space-y-1">
                  ...
                  <div className="grid grid-cols-2 gap-4 py-4 border-b border-luxury-gray-200">
                    <span className="text-sm font-light text-luxury-gray-600 uppercase tracking-wider">
                      {t('specs.boxAndPapers')}
                    </span>
                    <span className="text-sm font-light text-luxury-black text-right">
                      {product.specifications.boxAndPapers}
                    </span>
                  </div>
                  <div className="grid grid-cols-2 gap-4 py-4 border-b border-luxury-gray-200">
                    <span className="text-sm font-light text-luxury-gray-600 uppercase tracking-wider">
